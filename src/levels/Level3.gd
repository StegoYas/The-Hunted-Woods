...
func _ready():
	connect("noise_made", Callable($Helpers/EnemyDirector, "_on_noise_made"))
	director.setup(player)
	connecting_branches()
	director.connect("scared_by_noise", Callable(heartbeat, "got_scared"))
...
func connecting_branches():
	for branch in $Objects/BranchesHolder.get_children():
		branch.connect("stepped_on", Callable($Helpers/EnemyDirector, "_on_Branches_stepped_on"))
