...
@onready var color = $Sprite2D.get_self_modulate()
...
func rest(val = true):
	if val:
		...
		$Sprite2D.set_self_modulate(Color(color.r + 3,color.g,color.b,color.a))
	else:
...
func rotate_arc_light(new_dir = Vector2.ZERO):
	...
	var closest_dir = "RIGHT"
	var max_dot = -1.0
	for dir_name in DIR_LIMITS.keys():
		var dir_vector = DIR[dir_name]
		var dot = new_dir.normalized().dot(dir_vector.normalized())
		if dot > max_dot:
			max_dot = dot
			closest_dir = dir_name
	
	$ArcLightPosition.rotation_degrees = (DIR_LIMITS[closest_dir][0] + DIR_LIMITS[closest_dir][1])/2
		...
		match _state:
			STATES.IDLE, STATES.WALK:
				...
				if _state == STATES.IDLE:
					if angle > DIR_LIMITS["RIGHT"][0] and angle < DIR_LIMITS["RIGHT"][1]:
						change_facing_dir(DIR.RIGHT)
					elif angle > DIR_LIMITS["DOWN"][0] and angle < DIR_LIMITS["DOWN"][1]:
						change_facing_dir(DIR.DOWN)
					elif angle > DIR_LIMITS["LEFT"][0] and angle < DIR_LIMITS["LEFT"][1]:
						change_facing_dir(DIR.LEFT)
					elif angle > DIR_LIMITS["UP"][0] and angle < DIR_LIMITS["UP"][1]:
